...
export default function MyPost() {
  ...
  const delBlog = async (id) => {
    let result = await fetch(`http://localhost:5000/blog/deleteblog/${id}`, {
      method: "Delete",
      headers: {
  ...
  const fetchdata = async () => {
    try {
      ...
      const response = await fetch(
        `http://localhost:5000/blog/mypost/${email}`,
        {
          method: "GET",
          headers: {
      ...
      <div className="row" style={{ justifyContent: "center" }}>
        {filteredCards.length ? (
          ...
          <div style={{ color: "dimgrey" }}>
            <h4>No Such Blog</h4>
            <p style={{color:"red"}}>Note: If you want to show your blog than please fillup contact form. after that you are able to add blogs and also edit your blog here.</p>
          </div>
        )}
      </div>
      <Footer />
